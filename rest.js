...
function rest(key, secret, nonceGenerator) {
    ...
    //Ekliptor> sequential nonce requests
...
rest.prototype.make_request = function (path, params, cb) {
    //Ekliptor> sequential nonce requests
    this.requestParams.push({path: path, params: params, cb: cb});
    return new Promise((resolve, reject) => {
        // will return a promise with the underlying request object
        // the callback will still hold the response data, so it can be used as a drop-in replacement
        // (as long as you don't require the request object to be returned sync)
        this.requestQueue = this.requestQueue.then(() => { // only do 1 request at a time because of nonce
            return new Promise((resolve, reject) => {
                let nextParams = this.requestParams.shift();
                if (!nextParams)
                    return reject("No more parameters available for next request")
                path = nextParams.path;
                ...
                let reqObj = request({
                    ...
                    headers,
                    timeout: 15000,
                    forever: true // Ekliptor> HTTP keep-alive connection
                }, (err, response, body) => {
                    let error, result
                ...
                setTimeout(() => {
                    resolve(reqObj) // TODO: if we still get "nonce is too small" errors increase this timeot
                }, 450)
            })
        }).then((res) => {
            resolve(res) // forward the result (request object)
        }).catch((err) => {
...
rest.prototype.new_order = function (symbol, amount, price, exchange, side, type, is_hidden, postOnly, cb) {
    ...
    if (postOnly) {
        params['is_postonly'] = true // Ekliptor> fixed parameter name
    }
    if (is_hidden) {
