...
class BitfinexWS2 extends EventEmitter {
    ...
    open() {
        // Ekliptor> Crash Fix
        // also updated ws in package.json to "^4.0.0"
        try {
    ...
    _processBookEvent(msg, event) {
        ...
        // Ekliptor> Funding Features
        let eventName = event.symbol && event.symbol[0] === "f" ? "orderbookFunding" : "orderbook";
        this.emit(eventName, event.symbol, res)
        // Ekliptor< Funding Features
    }
    ...
    _processTradeEvent(msg, event) {
        ...
        // Ekliptor> Funding Features
        let eventName = event.symbol && event.symbol[0] === "f" ? "tradeFunding" : "trade";
        this.emit(eventName, event.symbol, res)
        // Ekliptor< Funding Features
    }
    ...
    subscribeFundingTrades(symbol = 'fBTC') {
        this.send({
            event: 'subscribe',
            channel: 'ftrades',
            symbol
